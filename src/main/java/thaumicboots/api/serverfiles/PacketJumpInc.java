...
public class PacketJumpInc implements IMessage, IMessageHandler<PacketJumpInc, IMessage> {
    ...
    @Override
    public IMessage onMessage(PacketJumpInc message, MessageContext ctx) {
        EntityPlayerMP player = ctx.getServerHandler().playerEntity;
        final ItemStack boots = IBoots.getBoots(player);
        if (boots != null && boots.getItem() instanceof IBoots item) {
            double jumpState = item.changeJump(boots, Config.bootsChangeRate);
